...
def find_all_files_with_formats_from_files(all_files, formats=('.webm',)):
    ...
    :return:
    """
    logger.debug("Finding webms in files.")
    ...
    logger.debug("Finding webms inf files: complete.")
    return result
...
def find_threads_by_word_in_comments(with_words=(), without_words=()):
    ...
    :return:
    """
    logger.debug("Finding thread by word in comments(word_to_find={}).".format(with_words))

    logger.debug("Getting response from {}".format(CATALOG_URL))
    ...
    logger.debug("Finding thread by word in comments: complete.")
    return result


def _get_threads_with_word_in_comments(all_threads, words=()):
    logger.debug("Getting thread with words({}).".format(words))
    ...
    logger.debug("Getting thread with words: complete.")
    return result


def _get_threads_without_word_in_comments(all_threads, words=()):
    logger.debug("Getting thread without words({}).".format(words))
    ...
    logger.debug("Getting thread without words: complete.")
    return result
...
def find_files_in_threads(threads_url):
    ...
    logger.debug("Finding files in threads({})".format(threads_url))

    result = []
    for thread_url in threads_url:
        result.extend(_find_files_in_thread(thread_url))

    logger.debug("Finding files in threads: complete.")
    return result
...
def _find_files_in_thread(thread_url):
    ...
    logger.debug("Finding files in thread({})".format(thread_url))

    logger.debug("Getting response from {}.".format(thread_url))
    response = urllib2.urlopen(thread_url)
    thread = json.loads(response.read().decode('utf-8'))

    logger.debug("Finding files in response.")
    result = []
    for post in filter(lambda x: x['files'], thread['threads'][0]['posts']):
        founded_files = post['files']
        result.extend(founded_files)
        logger.debug("Files found: {}".format(founded_files))

    logger.debug("Finding files in thread: complete.")
    return result
...
def _download_from_url(url):
    ...
    logger.debug("Downloading from: complete.")
    return result
